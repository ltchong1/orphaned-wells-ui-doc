...
In addition to the frontend and backend code, OGRRE relies on a Mongo database and Google Cloud Platform. Follow the other setup tutorials to learn how to configure MongoDB and GCP.
...
- <b>REACT_APP_STATE</b>: The collaborator that this instance is designed for. Each collaborator has their own set of trained processors, and OGRRE uses this variable to determine which processors to look for. For more information, see the section on processors [#TODO: add link here].
...
Prerequisites: Python and conda. See https://www.python.org/ and https://anaconda.org/anaconda/conda.
...
- <b>ENVIRONMENT</b>: The collaborator that this instance is designed for. Each collaborator has their own set of trained processors, and OGRRE uses this variable to determine which processors to look for. For example, "isgs".
- <b>BACKEND_URL</b>: The backend URL. If running the backend locally, this is likely going to be "http://localhost:8001". If running from a server with a dedicated IP/URL, use the proper address here.
